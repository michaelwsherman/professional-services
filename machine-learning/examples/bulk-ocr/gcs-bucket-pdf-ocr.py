...
Settings available by running gcs_bucket_pdf_ocr.py -h
  ...
  --batch-size 100 \
  --num-workers 50 \
  --output-report-file pdf_ocr_report.csv
"""
...
def async_detect_document_wrapper(gcs_source_uri, gcs_destination_uri,
  ...
  Args:
    ...
    gcs_service_account_path: string of a local path to a GCP 
  ...
  os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = gcs_service_account_path
  report_entry = {}
...
def ocr_uri_list_threaded(uri_list_to_ocr, gcs_destination_folder, num_workers,
                          reporting_dict, gcs_service_account_path,
  ...
  for _, value in return_dict.iteritems():
...
def main(args):
  ...
  logging.info("Arguments: gcs-source-uri %s --gcs-destination-uri %s " 
               "--batch-size %s --num-workers %s --output-report-file %s",
  ...
  for batch_number, batch in enumerate(batched_uris_to_ocr):
    ...
    ocr_uri_list_threaded(batch, args.gcs_destination_uri, args.num_workers,
                          reporting_dict, args.service_account_path, 
...
if __name__ == "__main__":
  ...
  parser.add_argument("--num-workers",
                      help=("number of workers (python processes) "
                            "to run simultaneously, default 50"),
                      type=int,
                      default=50)
